...
public static class Extends
{
    ...
    private static readonly Regex MobileUa =
        new(
            @"(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino",
            RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Compiled);

    private static readonly Regex MobileVersions =
        new(
    ...
    private static IHostBuilder BareApp<T>(string[]? args = null, int port = -1) where T : class
    {
        return Host.CreateDefaultBuilder(args)
            ...
            {
                // PLACEHOLDER: We can load more configuration data here.
                _ = config.Build().GetValue<string>("AppId");
                config.AddInMemoryCollection(new List<KeyValuePair<string, string>>
                {
                    new("SampleConfiguration", "SampleValue")
                });
            })
